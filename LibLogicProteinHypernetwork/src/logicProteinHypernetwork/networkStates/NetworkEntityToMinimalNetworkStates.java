 ...
 */
public class NetworkEntityToMinimalNetworkStates implements
		Transformer<NetworkEntity, Collection<MinimalNetworkState>> {
	 ...
	 *            the perturbations
	 */
	public NetworkEntityToMinimalNetworkStates(
			ProteinHypernetwork hypernetwork,
			Collection<NetworkEntity> perturbations) {
		this.hypernetwork = hypernetwork;
		networkEntityToFormula = new NetworkEntityToMinimalNetworkStateFormula(
				hypernetwork, perturbations);
	}
	 ...
	 */
	public void calculateAllTableauPaths(final NetworkEntity e,
			final Formula<NetworkEntity> f,
		...
		while (!blocked.isEmpty()) {
			Tableau<NetworkEntity> tableau = new Tableau<NetworkEntity>(
					tableauRules, propositionComparator, false);
			...
			if (!satisfiable && first) {
				System.err
						.println("Formula not satisfiable for entity "
								+ e
			...
			if (satisfiable) {
				...
				for (LabelledFormula<NetworkEntity> d : tableau
						.getDisjunctions()) {
					Formula<NetworkEntity> active = tableau
							.getActiveDisjunct(d).getFormula();
					...
					if (index != 0) {
						...
						for (Formula<NetworkEntity> child : d.getFormula()) {
							LabelledFormula<NetworkEntity> lf = tableau
									.getLabelledFormula(d.getWorld(), child);
							if (lf != null) {
								for (Formula<NetworkEntity> el : lf
										.getEliminationExplanation()) {
									if (el.indexOf(tableau.getActiveDisjunct(
											tableau.getLabelledFormula(
													d.getWorld(), el))
											.getFormula()) == 0) {
										Collection<Formula<NetworkEntity>> bs2 = new ArrayList<Formula<NetworkEntity>>(
												bs);
										bs2.add(tableau.getActiveDisjunct(d)
												.getFormula());
										blocked.push(bs2);
									}
								}
